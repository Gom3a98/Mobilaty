...
def index_1():
    ...
    response = jsonify(post.show_post(request.args))
    response.status_code = 200
    return response

@app.route('/post/findByStoreId', methods = ['GET'])
def findByStoreId():
    response = jsonify(post.findByStoreId(request.args))
    response.status_code = 200
    return response
